        ...
        <div class="card-body p-0">
            <div class="p-5">
                <div class="text-center">
                    <h4 class="text-dark mb-4">New Ram Option</h4>
                </div>
                <p id="error" style="text-align: center;color: rgb(255,0,0);background: rgb(255,216,216);"></p>
                <form id="form" class="user">
                    <div class="row mb-3">
                        <div class="col-sm-6 mb-3 mb-sm-0">Name<br/><input class="ram_optionname form-control form-control-user" type="text" min="0" id="freq" placeholder="Name" name="name" required></div>
...
<script type="text/javascript">
 const urlredirect = '<%= request.getContextPath()+"/table/ram_option/1" %>';
 const url = '<%= request.getContextPath()+"/save/ram_option" %>';
 const formulaire = document.getElementById('form');

 formulaire.addEventListener('submit', function (event) {
      event.preventDefault(); // Empêche le formulaire d'être soumis
      const name = Array.from(document.getElementsByClassName("ram_optionname")).map(option => option.value);
